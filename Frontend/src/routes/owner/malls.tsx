...
import { MapPin, Bike, Car, Plus, Layers, CheckCircle, IndianRupee } from "lucide-react";
...
function OwnerMalls() {
  ...
  useEffect(() => {
    (async () => {
      try {
        const [mallsRes, statsRes] = await Promise.all([
          api.get("/owner/malls"),
  ...
  return (
    <div className="bg-[#F5F3EE]">
      ...
      <section className="mx-auto max-w-7xl px-4 py-14 sm:px-6 lg:px-8">
        {loading ? (
          <Loader text="Loading Your Malls" />
        ) : (
          <>
            {globalStats && (
              <div className="mb-14 grid gap-5 sm:grid-cols-2 lg:grid-cols-4">
                {[
                  { label: "Available Slots", value: globalStats.availableSlots, Icon: CheckCircle },
                  { label: "Occupied Slots", value: globalStats.occupiedSlots, Icon: Car },
                  { label: "Today's Revenue", value: `₹${globalStats.todaysRevenue}`, Icon: IndianRupee, accent: true },
                  { label: "Total Revenue", value: `₹${globalStats.totalRevenue}`, Icon: Layers },
                ].map((c, i) => (
                  <motion.div
                    key={c.label}
